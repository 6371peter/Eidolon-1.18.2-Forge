...
public class SlashParticle extends TextureSheetParticle {
    ...
    public SlashParticle(ClientLevel world, SlashParticleData data, double x, double y, double z, double vx, double vy, double vz) {
    	...
    	x -= sy * cp * data.width * 0.25f; 
    	y -= sp * data.width * 0.25f;
    	z -= cy * cp * data.width * 0.25f;
    	setPos(x, y, z);
        this.xd = vx;
        this.yd = vy;
        this.zd = vz;
    ...
    public void render(VertexConsumer vc, Camera info, float pticks) {
		...
		for (float i = 0; i < 18; i ++) {
	        ...
	        // vert
	        decoVert(b.vertex(ox + dx2 * r + yax * w2 * 0.5f, oy + dy2 * r + yay * w2 * 0.5f, oz + dz2 * r + yaz * w2 * 0.5f), u1, v1, al2, lmap);
	        decoVert(b.vertex(ox + dx2 * r + yax * -w2 * 0.5f, oy + dy2 * r + yay * -w2 * 0.5f, oz + dz2 * r + yaz * -w2 * 0.5f), u1, v0, al2, lmap);
